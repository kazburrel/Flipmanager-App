        ...
        <div class="toolbar" id="kt_toolbar">
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">File Manager - Folders</h1>
                        ...
                        <table id="kt_manager_list" class="table align-middle table-row-dashed fs-6 gy-5">
                            <thead>
                                <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
                                    ...
                                    <th class="min-w-155px text-center">Course it belongs to</th>
                                    <th class="w-125px"></th>
                            ...
                            <tbody class="fw-bold text-gray-600">
                                @forelse ($folders as $folder)
                                    <tr>
                                        ...
                                        <td class="text-center" data-order="{{ $folder->name }}">
                                            <div class="d-flex align-items-center">
                                                ...
                                                <a href="{{ route('admin.viewFolder.file', $folder->id) }}"
                                                    class="text-gray-800 text-hover-primary">{{ $folder->name }}</a>
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            @php
                                                $size = $folderSize->firstWhere('id', $folder->id)->size ?? null;
                                                if ($size === null) {
                                                    echo 'N/A';
                                                } else {
                                                    if ($size >= 1073741824) {
                                        ...
                                        <td class="text-center">{{ $folder->category->name }}</td>
                                        <td class="text-end " data-kt-filemanager-table="action_dropdown">
                                        <td class="text-center">
                                            <a href="#"
                                                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1"
                                                data-bs-toggle="modal"
                                                data-bs-target="#kt_modal_edit_folder_{{ $folder->id }}" title="Edit">
                                                <i class="bi bi-pencil-fill text-primary"></i>
                                            </a>
                                            <a href="#"
                                                class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm me-1"
                                                data-bs-toggle="modal" data-bs-target="#deleteModal_{{ $folder->id }}"
                                                title="Delete">
                                    ...
                                    <div class="modal fade" id="kt_modal_edit_folder_{{ $folder->id }}" tabindex="-1"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered mw-650px">
                                            <div class="modal-content">
                                                <form class="form"
                                                    action="{{ route('admin.folder.update', $folder->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <div class="modal-header">
                                                        <h2 class="fw-bolder">Edit Folder</h2>
                                                    ...
                                                    <div class="modal-body pt-10 pb-15 px-lg-17">
                                                        <div class="form-group">
                                                            <label for="folder_name_{{ $folder->id }}"
                                                                class="form-label">Folder Name</label>
                                                            <input type="text" class="form-control form-control-solid"
                                                                id="folder_name_{{ $folder->id }}" name="name"
                                                                value="{{ $folder->name }}"
                                                        ...
                                                        <div class="form-group mt-5">
                                                            <label for="course_{{ $folder->id }}"
                                                                class="form-label">Course</label>
                                                            <select class="form-control form-control-solid"
                                                                id="course_{{ $folder->id }}" name="category_id">
                                                                <option value="">Select course</option>
                                                                @foreach ($courses as $course)
                                                                    <option value="{{ $course->id }}"
                                                                        {{ $course->id == $folder->category_id ? 'selected' : '' }}>
                                                                        {{ $course->name }}</option>
                                                                @endforeach
                                                            </select>
                                                            @error('course')
                                    ...
                                    <div class="modal fade" id="deleteModal_{{ $folder->id }}" tabindex="-1"
                                        aria-labelledby="deleteModalLabel_{{ $folder->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="deleteModalLabel_{{ $folder->id }}">
                                                        Confirm Deletion</h5>
                                                ...
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                        data-bs-dismiss="modal">Cancel</button>
                                                    <form method="POST"
                                                        action="{{ route('admin.folder.destroy', $folder->id) }}">
                                                        @csrf
                                                        @method('DELETE')
